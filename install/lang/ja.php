...
return [
    ...
    'quick_login_url' => '管理画面 URL',
    'login_info_title' => '管理画面のログイン情報',
    'login_info_pass_hint' => '前のステップでご自身が設定したパスワード',
    'login_info_tip' => 'この URL をブックマークしてください。パスワードを忘れた場合はデータベースから再設定できます。',
    'quick_copy' => 'コピー',
    'quick_copied' => 'コピーしました',
    'quick_copy_fail' => '手動でコピー',
    'quick_copy_all' => 'すべてコピー',
    ...
    'rewrite_nginx_wp_title' => '推奨：同梱ルールを include（セキュリティ遮断を含む）',
    'rewrite_nginx_wp' => 'BTパネル等：リライト設定に <code>include /www/wwwroot/サイトディレクトリ/deploy/nginx-baota.conf;</code> の 1 行だけを書きます。アップグレード後も自動で更新されます。「wordpress」プリセットだけを選ばないでください——storage/database.sqlite のデータベースを誰でもダウンロードできてしまいます。',
    'rewrite_nginx_desc' => 'server ブロックを手書きする場合は、次の 2 つを追加してください（1 つ目で機密ディレクトリを遮断）：',
    'rewrite_nginx_advanced' => '上級者向け（静的 HTML 直接配信＋サーバー側の保護）はリポジトリの deploy/nginx-server.conf を参照。',
    'rewrite_nginx_reload' => '設定後、nginx -t で構文チェックし、nginx -s reload で反映してください。反映後に /storage/database.sqlite を開き、403 または 404 になることを確認してください。',
    ...
    'error_admin_user_invalid' => '管理者アカウントは空や「0」にできません。前後に空白も入れないでください。',
    'error_db_name_invalid' => 'データベース名に使えるのは英数字、アンダースコア、ハイフンのみです。',
    'error_table_prefix_invalid' => 'テーブル接頭辞に使えるのは英数字とアンダースコアのみです。',
    'error_db_not_exists' => 'データベース「:name」が存在しません。「データベースが存在しない場合は作成する」にチェックを入れて再度お試しください。',
