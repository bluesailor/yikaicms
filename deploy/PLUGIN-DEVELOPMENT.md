...
本文根据当前主仓源码整理，不是 WordPress 插件教程。源码核对基线：v1.20.0 发布提交 `abbac237c9ef9dc6dc3671bfd88f33f0b20b56b2`（0.2 版基线为 1.19.9 `c97ca429`）。插件加载器、钩子 API 与后台插件路由在两版之间未变；1.20.0 新增的是安装器、来源回执与市场安装规则（见第 9 节）。代码须兼容 PHP 8.0（产品运行下限），推荐 8.2+；数据库兼容 MySQL 5.7 / MariaDB 10.x 与 SQLite。
