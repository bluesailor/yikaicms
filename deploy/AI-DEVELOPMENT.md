...
更新：2026-09-23，按 v2.0.0（`95ff9bedecdd454a6998195ce025afd32c946460`）核对。适用于不同 AI 编程助手和人工开发者，不依赖特定工具、账号或本机路径。
...
v1.20.1 起，易开网页构建器的循环模板、显示条件、全局样式、表格、价格方案为专业授权能力，由 `yikai-builder`（易开网页构建器 Pro）插件提供，经插件市场分发、不随完整包。插件与主题不要依赖这五项能力，也不要复制或绕过它们的授权检查（`BloxFeaturePolicy`）。
