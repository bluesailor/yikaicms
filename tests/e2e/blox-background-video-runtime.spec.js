...
async function openRuntimePage(page, mobileMode = 'video') {
  await page.route('**/__blox-background-video-runtime', route => route.fulfill({
    contentType: 'text/html',
    body: `<!doctype html><html><body>
      <video muted loop playsinline preload="none" data-blox-background-video
...
test('dynamic replacement releases the old video and starts the new node @ci', async ({ page }) => {
  ...
  const result = await page.evaluate(videoUrl => {
    ...
    return {
      ...
      newSrc: replacement.getAttribute('src'),
      calls: window.__bloxMediaCalls,
      expected: videoUrl,
    };
  }, VIDEO_URL);

  expect(result.oldSrc).toBeNull();
  expect(result.newSrc).toBe(result.expected);
  expect(result.calls.pause).toBeGreaterThan(0);
  expect(result.calls.load).toBeGreaterThan(0);
  expect(result.calls.play).toBeGreaterThanOrEqual(2);
});
