...
const { openEditor, frame, waitPreviewSettled, undo, expectClean } = require('./helpers');
...
async function publicLanguages(browser, baseURL, urls, testInfo) {
  ...
  try {
    for (const [lang, url, title, body, caption, button] of languages) {
      ...
      await expect(page.getByRole('link', { name: button, exact: true }).first()).toHaveAttribute('href', urls[lang].href);
      if (lang !== 'zh-CN') {
        await expect(page.getByText('为客户提供专业服务。', { exact: true })).toHaveCount(0);
      }
      await heading.scrollIntoViewIfNeeded();
      const overlay = page.locator('.yk-div-overlay').first();
      ...
      const navigation = page.waitForResponse(response => response.request().isNavigationRequest()
        && response.url() === new URL(urls[lang].destination, baseURL).href);
      await page.getByRole('link', { name: button, exact: true }).first().click();
      expect((await navigation).status()).toBe(200);
      await expect(page).toHaveURL(new URL(urls[lang].destination, baseURL).href);
      await expect(page.locator('html')).toHaveAttribute('lang', lang);
    }
...
test('classic conversion publishes one ordinary layout with three localized contents @ci', async ({ page, browser, baseURL }, testInfo) => {
  ...
  await undo(page);
  await expect(page.locator('[data-testid="blox-tree-element"][data-home-block-type="about"]')).toHaveCount(1);
  await expectClean(page);
  await section.locator('[data-section-drag-handle]').first().click();
  await section.locator('[data-home-column-tree$=".text"]').click();
  await page.getByTestId('blox-convert-about').click();
  await expect(page.locator('[data-testid="blox-tree-element"][data-home-block-type="about"]')).toHaveCount(0);
  const saved = page.waitForResponse(response => response.url().includes('/blox_home_api.php')
    && response.request().method() === 'POST'
    && !new URLSearchParams(response.request().postData() || '').has('action'));
