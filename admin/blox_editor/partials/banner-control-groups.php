...
<template x-if="BloxBannerPanel.supports(selEl) && panelTab === 'content' && !ctrlQuery.trim() && !modifiedOnly">
    ...
    <div role="group" aria-label="<?= e(__('blox_banner_settings_groups')) ?>" class="flex gap-1 border-b border-gray-200 pb-2">
        <?php foreach (['common' => 'blox_banner_group_display', 'playback' => 'blox_banner_group_playback', 'motion' => 'blox_banner_group_motion', 'mobile' => 'blox_banner_group_mobile'] as $group => $label): ?>
        <button type="button" @click="bannerPanelGroup = '<?= e($group) ?>'"
            ...
            <span x-text="selEl.type === 'home-banner-item' ? <?= e($jt(['common' => 'blox_banner_group_content', 'playback' => 'blox_banner_group_links', 'motion' => 'blox_banner_group_motion', 'mobile' => 'blox_banner_group_mobile'][$group])) ?> : <?= e($jt($label)) ?>"></span>
        </button>
        <?php endforeach; ?>
    </div>
