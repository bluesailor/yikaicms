        ...
        <div x-show="professionalRelevant('<?= e($feature) ?>')" data-professional-feature="<?= e($feature) ?>">
            <template x-if="professionalFeatures['<?= e($feature) ?>'].allowed">
                <button type="button" @click="openProfessionalFeature('<?= e($feature) ?>')"
                    ...
                    <span><?= e(__($label)) ?></span><?php require __DIR__ . '/pro-badge.php'; ?><i class="ti ti-chevron-right" aria-hidden="true"></i>
                </button>
            </template>
            <template x-if="!professionalFeatures['<?= e($feature) ?>'].allowed">
                <div class="space-y-1">
                    <span class="inline-flex items-center gap-1 font-medium text-gray-700"><?= e(__($label)) ?><?php require __DIR__ . '/pro-badge.php'; ?></span>
