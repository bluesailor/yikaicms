...
// r41：Blox 成为唯一受支持的可视化页面编辑器。旧文件暂留用于回滚诊断，
// 但不再接受正式导航和直接访问；诊断旁路只开放给本地 DEBUG 超级管理员。
$legacyDebugMode = defined('DEBUG') && DEBUG
    && hasPermission('*')
...
if (!$legacyDebugMode && ($id > 0 || $isHomeLayout)) {
    if ($isHomeLayout) {
        ...
        if ($focus !== '') {
            $target .= '&focus=' . rawurlencode($focus);
        }
    }
...
if ($isHomeLayout) {
    ...
    $storedBlocksData = trim((string) ($contentRecord['blocks_data'] ?? '')) !== ''
        ? (string) $contentRecord['blocks_data']
        : '[]';
    $pageBaseRevision = BloxDocumentPipeline::fingerprint($storedBlocksData);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'preview') {
    ...
    outputBloxCanvasPreview($isHomeLayout, $id);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $currentBlocksData = trim((string) ($contentRecord['blocks_data'] ?? '')) !== ''
        ? (string) $contentRecord['blocks_data']
        : '[]';
    if ($baseRevision !== '' && !BloxDocumentPipeline::revisionMatches($currentBlocksData, $baseRevision)) {
    ...
    success([
        'base_revision' => BloxDocumentPipeline::fingerprint($postBlocksData),
    ]);
}
...
<?php else: ?>
    <input type="hidden" name="base_revision" value="<?php echo e($pageBaseRevision); ?>">
<?php endif; ?>
<?php if (!$isHomeLayout): ?>
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b">
...
<?php
// 初始化区块数据：高级编辑器同样只接收迁移后的 sections，不能把 v1 信封
// 整个塞进 Alpine 的 sections。坏 JSON/未来版本硬拒绝，避免空画布覆盖原文档。
$blocksDataJson = $blocksData ?: '[]';
try {
    $loadedDocument = BloxDocumentPipeline::decode($blocksDataJson);
} catch (RuntimeException $documentError) {
    http_response_code(409);
    exit(e($documentError->getMessage()));
}
$blocksDataJson = json_encode(
    $loadedDocument['sections'],
    JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT
) ?: '[]';
...
document.getElementById("editForm").addEventListener("submit", async function(e) {
    ...
    try {
        ...
        if (result.code === 0) {
            ...
            var revisionEl = this.querySelector("input[name=base_revision]");
            if (revisionEl && result.data && result.data.base_revision) {
                revisionEl.value = result.data.base_revision;
            }
    ...
    async function run(action, confirmText, btn) {
        ...
        try {
            ...
            if (action === "publish") {
                ...
                var revisionEl = form.querySelector("input[name=base_revision]");
                if (revisionEl && saveResult.data && saveResult.data.base_revision) {
                    revisionEl.value = saveResult.data.base_revision;
                }
            }
