...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        echo json_encode(['code' => 0, 'msg' => __('admin_saved')], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    if ($action === 'clear') {
        ...
        echo json_encode(['code' => 0, 'msg' => str_replace(':n', (string) $count, __('scache_cleared'))], JSON_UNESCAPED_UNICODE);
        exit;
    }
...
// TTL 预设（秒 => 标签）
$ttlPresets = [
    60    => '1 ' . __('scache_min_debug'),
    300   => '5 ' . __('scache_min_news'),
    600   => '10 ' . __('scache_min_content'),
    1800  => '30 ' . __('scache_min_content_long'),
    3600  => '1 ' . __('scache_hour_corp'),
    7200  => '2 ' . __('scache_hour_corp_long'),
    21600 => '6 ' . __('scache_hours'),
    43200 => '12 ' . __('scache_hours'),
    86400 => '24 ' . __('scache_hour_static'),
];

// 友好显示当前 TTL
$ttlDisplay = $ttl < 60 ? $ttl . ' ' . __('scache_sec')
    : ($ttl < 3600 ? round($ttl/60, 1) . ' ' . __('scache_min')
    : ($ttl < 86400 ? round($ttl/3600, 1) . ' ' . __('scache_hours')
    : round($ttl/86400, 1) . ' ' . __('scache_days')));
...
<div class="p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800"><?php echo e(__('scache_title')); ?></h1>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="text-2xl font-bold <?php echo $enabled ? 'text-green-600' : 'text-gray-400'; ?>">
                <?php echo $enabled ? e(__('scache_on')) : e(__('scache_off')); ?>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-xs text-gray-500 uppercase tracking-wide mb-1"><?php echo e(__('scache_files')); ?></div>
            <div class="text-2xl font-bold text-gray-800"><?php echo number_format($fileCount); ?></div>
            <div class="text-xs text-gray-400 mt-0.5"><?php echo str_replace(':size', formatFileSize($totalSize), e(__('scache_total_size'))); ?></div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-xs text-gray-500 uppercase tracking-wide mb-1"><?php echo e(__('scache_ttl')); ?></div>
            <div class="text-2xl font-bold text-gray-800"><?php echo htmlspecialchars($ttlDisplay); ?></div>
            <div class="text-xs text-gray-400 mt-0.5"><?php echo $ttl; ?> <?php echo e(__('scache_sec')); ?></div>
        </div>
    </div>
        ...
        <form id="cacheForm" onsubmit="event.preventDefault(); saveCache();">
            <div class="space-y-5">
                <label class="flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                        <div class="font-medium text-gray-800"><?php echo e(__('scache_enable')); ?></div>
                        <div class="text-sm text-gray-500 mt-0.5"><?php echo e(__('scache_enable_tip')); ?></div>
                    </div>
                </label>

                <div>
                    <label class="block font-medium text-gray-800 mb-1"><?php echo e(__('scache_ttl')); ?></label>
            ...
            <div class="mt-6 flex gap-3">
                <button type="submit" class="bg-primary hover:opacity-90 text-white px-6 py-2 rounded"><?php echo e(__('save_settings')); ?></button>
                <button type="button" onclick="clearCache()" class="border border-gray-300 hover:bg-gray-50 px-6 py-2 rounded text-red-600"><?php echo e(__('scache_clear_now')); ?></button>
            </div>
        </form>
    </div>
...
async function saveCache() {
    ...
    showMessage(d.msg || (d.code === 0 ? <?php echo json_encode(__('admin_saved'), JSON_UNESCAPED_UNICODE); ?> : <?php echo json_encode(__('admin_failed'), JSON_UNESCAPED_UNICODE); ?>), d.code === 0 ? 'success' : 'error');
    if (d.code === 0) setTimeout(() => location.reload(), 600);
}

async function clearCache() {
    if (!confirm(<?php echo json_encode(__('scache_clear_confirm'), JSON_UNESCAPED_UNICODE); ?>)) return;
