...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_status') {
        $id = postInt('id');
        $newStatus = articleModel()->toggle($id, 'status');
        success(['status' => $newStatus]);
    }

    if ($action === 'toggle_top') {
        $id = postInt('id');
        $newValue = articleModel()->toggle($id, 'is_top');
        success(['is_top' => $newValue]);
    }

    if ($action === 'toggle_recommend') {
        $id = postInt('id');
        $newValue = articleModel()->toggle($id, 'is_recommend');
...
$result = articleModel()->getAdminList($filters, $perPage, $offset);
$total = $result['total'];
...
<div class="bg-white rounded-lg shadow mb-6">
    <div class="flex border-b">
        <a href="/admin/article.php" class="px-6 py-3 text-sm font-medium border-b-2 border-primary text-primary">文章列表</a>
        <a href="/admin/article_category.php" class="px-6 py-3 text-sm font-medium text-gray-500 hover:text-gray-700 border-b-2 border-transparent hover:border-gray-300">分类管理</a>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow mb-6">
    <div class="p-4 flex flex-wrap gap-4 items-center justify-between">
        <form class="flex flex-wrap gap-3 items-center">
            <select name="category_id" class="border rounded px-3 py-2">
                ...
                <option value="<?php echo $cat['id']; ?>" <?php echo $categoryId === (int)$cat['id'] ? 'selected' : ''; ?>>
                    <?php echo $cat['_prefix'] . e($cat['name']); ?>
                </option>
                <?php endforeach; ?>
            </select>
        ...
        <a href="/admin/article_edit.php" class="bg-primary hover:bg-secondary text-white px-4 py-2 rounded inline-flex items-center gap-1">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center text-sm text-gray-500">
                            <?php echo $item['publish_time'] ? date('Y-m-d H:i', (int)$item['publish_time']) : '-'; ?>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <a href="/admin/article_edit.php?id=<?php echo $item['id']; ?>"
                               class="text-primary hover:underline text-sm mr-2 inline-flex items-center gap-1">
                                ...
                                编辑</a>
                            <button onclick="deleteArticle(<?php echo $item['id']; ?>)"
