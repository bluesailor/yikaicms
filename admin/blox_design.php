...
<div class="min-w-0 max-w-full space-y-7 overflow-hidden" x-data="bloxDesignManager()" data-testid="blox-design-page">
    ...
    <div x-show="notice" x-cloak class="border-l-4 border-emerald-500 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status" x-text="notice"></div>
    <div x-show="error" x-cloak class="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-800" role="alert" x-text="error"></div>
    ...
    <section x-show="tab === 'colors'" data-testid="blox-design-page-colors">
        ...
        <form @submit.prevent="addToken()" class="grid min-w-0 max-w-full gap-3 border-y border-gray-200 bg-gray-50 px-4 py-4 md:grid-cols-[minmax(0,1.2fr)_minmax(0,.8fr)_minmax(0,1fr)_auto] md:items-end">
            ...
            <label class="block text-xs font-medium text-gray-600">
                ...
                <span class="mt-1 flex h-10 items-center gap-2 border border-gray-300 bg-white px-2">
                    <input type="color" x-model="newToken.value" class="h-7 w-9 border-0 bg-transparent p-0">
                    <input type="text" x-model="newToken.value" pattern="#[0-9a-fA-F]{6}" required class="min-w-0 flex-1 border-0 bg-transparent font-mono text-sm outline-none">
                </span>
            </label>
                ...
                <tbody class="divide-y divide-gray-100">
                    <template x-for="token in activeTokens()" :key="token.id">
                        <tr data-testid="blox-design-page-token-row">
                            ...
                            <td class="px-3 py-3">
                                <span class="flex h-9 min-w-44 items-center gap-2 border border-gray-300 px-2" :class="token.system || token.locked ? 'bg-gray-50' : 'bg-white'">
                                    <input type="color" x-model="token.value" :disabled="token.system || token.locked" class="h-6 w-8 border-0 bg-transparent p-0 disabled:opacity-50">
                                    <input type="text" x-model="token.value" :disabled="token.system || token.locked" class="min-w-0 flex-1 border-0 bg-transparent font-mono text-xs outline-none">
                                </span>
                            </td>
    ...
    <div x-show="!advanced" class="border-l-4 border-amber-400 bg-amber-50 px-4 py-3 text-sm text-amber-800" data-testid="blox-design-page-advanced-locked">
