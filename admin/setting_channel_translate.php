...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'ai_translate') {
        ...
        if (empty($toTranslate)) {
            success([], str_replace(':item', __('atr_item_channels'), __('atr_all_done')));
            exit;
        }
        ...
        if (!$result['success']) {
            error(str_replace(':error', (string) ($result['error'] ?? __('atr_unknown_error')), __('atr_ai_failed')));
        }
...
$pageTitle = str_replace(':item', __('atr_item_channels'), __('atr_title'));
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="divide-y">
                ...
                <div class="px-6 py-3 flex items-center gap-4 bg-blue-50/50" data-src-name="<?php echo e($zhName); ?>">
                    <div class="w-1/3 text-sm font-medium">
                        <?php echo e($zhName); ?>
                        <span class="text-xs text-blue-400 ml-1"><?php echo e(__('atr_system')); ?></span>
                    </div>
                    <div class="flex-1">
            ...
            <div class="px-6 py-4 border-t">
                <span class="text-xs text-gray-400">
                    <?php echo e(str_replace([':total', ':translated', ':item'], [(string) count($srcChannels), (string) count(array_filter($targetChannels)), __('atr_item_channels')], __('atr_count_summary'))); ?>
                </span>
            </div>
        </div>
...
async function aiTranslate() {
    ...
    if (empties.length === 0) {
        showMessage(<?php echo json_encode(str_replace(':item', __('atr_item_channels'), __('atr_no_empty')), JSON_UNESCAPED_UNICODE); ?>);
        return;
    }
    if (!confirm(<?php echo json_encode(str_replace(':item', __('atr_item_channels'), __('atr_ai_confirm')), JSON_UNESCAPED_UNICODE); ?>.replace(':n', empties.length))) return;
