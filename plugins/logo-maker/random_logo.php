...
function logoMakerRandomColorSchemes(): array
{
    return [
...
function logoMakerRandomMonoColors(): array
{
    return [
...
function logoMakerRandomMonoHexPalette(): array
{
    return [
...
function logoMakerRandomNormalizeHexColor(string $color, string $fallback): string
{
    $color = trim($color);
...
function logoMakerRandomColorModes(): array
{
    return [
 ...
 */
function logoMakerRandomApplyColorMode(array $colors, string $colorMode): array
{
...
function logoMakerRandomLetterStyles(): array
{
    return [
...
function logoMakerRandomEffects(): array
{
    return [
...
function logoMakerRandomBackgroundModes(): array
{
    return [
...
function logoMakerRandomIndustries(): array
{
    return [
        'technology' => [
            'label' => '科技 / 软件 / AI',
...
function logoMakerRandomSeed(string $industry, string $name, int $seed, int $index): int
{
    return abs((int) sprintf('%u', crc32($industry . '|' . $name . '|' . $seed . '|' . $index)));
}

function logoMakerRandomLetters(string $name): string
{
...
{
    $presets = logoMakerRandomIndustries();
    $schemes = logoMakerRandomColorSchemes();
    $letterStyles = logoMakerRandomLetterStyles();
    $effects = logoMakerRandomEffects();
    $backgroundModes = logoMakerRandomBackgroundModes();
    $colorModes = logoMakerRandomColorModes();
    $monoColors = logoMakerRandomMonoColors();
    $preset = $presets[$industry] ?? $presets['technology'];
    $randomSeed = logoMakerRandomSeed($industry, $name, $seed, $index);
    $seriesSeed = logoMakerRandomSeed($industry, $name, $seed, 0);
    $random = new LogoMakerRandom($randomSeed);
    $paletteRandom = new LogoMakerRandom($randomSeed ^ 0x5F3759DF);
    ...
    $customPalette = [
        logoMakerRandomNormalizeHexColor((string) ($customColors[0] ?? ''), '#1D4ED8'),
        logoMakerRandomNormalizeHexColor((string) ($customColors[1] ?? ''), '#22D3EE'),
        logoMakerRandomNormalizeHexColor((string) ($customColors[2] ?? ''), '#0F172A'),
    ];
    ...
    $selectedEffect = logoMakerRandomResolveEffect($industry, $name, $seed, $index, isset($effects[$effect]) ? $effect : 'auto');
    ...
    if ($selectedColorMode === 'mono' && $selectedMonoColor !== 'industry') {
        ...
        [$primary, $accent, $dark] = logoMakerRandomApplyColorMode($activePalette, $selectedColorMode);
    }
    ...
    $letters = logoMakerRandomLetters($name);
    ...
    $shape = logoMakerRandomContainerShape($container, $radius);
    $background = match ($selectedBackgroundMode) {
        'solid' => '<g clip-path="url(#' . $uid . 'shape)"><rect width="128" height="128" fill="url(#' . $uid . 'g)"/>'
            . logoMakerRandomBackgroundTreatment($treatment, $random, $accent) . '</g>'
            . logoMakerRandomContainerShape($container, $radius, 'fill="none" stroke="#fff" stroke-width="2" opacity=".12"'),
        'outline' => logoMakerRandomContainerShape($container, $radius, 'fill="none" stroke="' . e($primary) . '" stroke-width="4" opacity=".88"'),
        default => '',
    };

    $mark = match ($selectedLetterStyle) {
        'abstract' => logoMakerRandomAbstractTemplate($index, $random, $primary, $accent, $dark),
        'spatial' => logoMakerRandomSpatialTemplate($index, $random, $primary, $accent, $dark),
        'geometry' => logoMakerRandomLibraryTemplate($industry, $index, $seriesSeed, $random, $primary, $accent, $dark, $template, $letters, $uid),
        default => logoMakerRandomLetterTemplate($selectedLetterStyle, $letters, $accent, $dark, $uid, $random, $index),
    };
    ...
    $effectParts = logoMakerRandomEffectParts($selectedEffect, $mark, $random, $accent, $uid);
...
function logoMakerRandomAbstractTemplate(int $index, LogoMakerRandom $random, string $primary, string $accent, string $dark): string
{
    $primary = e($primary);
    $accent = e($accent);
    $dark = e($dark);
...
function logoMakerRandomSpatialTemplate(int $index, LogoMakerRandom $random, string $primary, string $accent, string $dark): string
{
    $primary = e($primary);
    $accent = e($accent);
    $dark = e($dark);
...
function logoMakerRandomResolveEffect(string $industry, string $name, int $seed, int $index, string $effect): string
{
    if ($effect !== 'auto' && isset(logoMakerRandomEffects()[$effect])) {
        return $effect;
    }
 ...
 */
function logoMakerRandomEffectParts(string $effect, string $mark, LogoMakerRandom $random, string $accent, string $uid): array
{
...
function logoMakerRandomLetterTemplate(
    string $style,
    string $letters,
    string $accent,
    string $dark,
    string $uid,
    LogoMakerRandom $random,
    int $index
): string {
    $text = e($letters);
    ...
    return match ($style) {
        'cut' => logoMakerRandomCutTemplate($text, $accent, $dark, $uid, $fontSize, $font, $index, $random),
...
function logoMakerRandomCutTemplate(
    string $text,
    string $accent,
    string $dark,
    string $uid,
...
function logoMakerRandomContainerShape(string $container, int $radius, string $attributes = ''): string
{
...
function logoMakerRandomBackgroundTreatment(string $treatment, LogoMakerRandom $random, string $accent): string
{
    $accent = e($accent);
 ...
 */
function logoMakerRandomIndustryIconMap(): array
{
    static $map = null;
    if (is_array($map)) {
        return $map;
...
function logoMakerRandomLibraryTemplate(
    ...
    $map = logoMakerRandomIndustryIconMap();
    $items = $map[$industry] ?? [];
    if ($items === []) {
    ...
    $count = count($items);
    $step = 5;
    while ($count > 1 && logoMakerRandomGreatestCommonDivisor($step, $count) !== 1) {
        $step += 2;
    }
...
function logoMakerRandomGreatestCommonDivisor(int $left, int $right): int
{
    while ($right !== 0) {
...
function logoMakerRandomTemplate(string $template, LogoMakerRandom $random, string $accent, string $letters, string $uid): string
{
    $letterSize = mb_strlen($letters, 'UTF-8') > 1 ? 34 : 50;
    return match ($template) {
        'orbit' => logoMakerRandomOrbit($random, $accent),
        'nodes' => logoMakerRandomNodes($random, $accent),
        'circuit' => logoMakerRandomCircuit($random, $accent),
        'pulse' => logoMakerRandomPulse($random, $accent),
        'mesh' => logoMakerRandomMesh($random, $accent),
        'hex' => logoMakerRandomHex($random, $accent, $uid),
        'blocks' => logoMakerRandomBlocks($random, $accent),
        'ring_cut' => logoMakerRandomRingCut($random, $accent),
        'gear' => logoMakerRandomGear($random, $accent),
        'chevron' => logoMakerRandomChevron($random, $accent),
        'roof' => logoMakerRandomRoof($random, $accent),
        'grid' => logoMakerRandomGrid($random, $accent),
        'bridge' => logoMakerRandomBridge($random, $accent),
        'tower' => logoMakerRandomTower($random, $accent),
        'cross' => logoMakerRandomCross($random, $accent),
        'cell' => logoMakerRandomCell($random, $accent),
        'wave' => logoMakerRandomWave($random, $accent),
        'dna' => logoMakerRandomDna($random, $accent),
        'heartpulse' => logoMakerRandomHeartPulse($random, $accent),
        'book' => logoMakerRandomBook($random, $accent),
        'shield_mark' => logoMakerRandomShieldMark($random, $accent),
        'spark' => logoMakerRandomSpark($random, $accent),
        'pencil' => logoMakerRandomPencil($random, $accent),
        'columns' => logoMakerRandomColumns($random, $accent),
        'bars' => logoMakerRandomBars($random, $accent),
        'coin' => logoMakerRandomCoin($random, $accent),
        'trend' => logoMakerRandomTrend($random, $accent),
        'ledger' => logoMakerRandomLedger($random, $accent),
        'leaf' => logoMakerRandomLeaf($random, $accent),
        'bowl' => logoMakerRandomBowl($random, $accent),
        'seed' => logoMakerRandomSeedMark($random, $accent),
        'sprout' => logoMakerRandomSprout($random, $accent),
        'wheat' => logoMakerRandomWheat($random, $accent),
        'petal' => logoMakerRandomPetal($random, $accent),
        'ribbon' => logoMakerRandomRibbon($random, $accent),
        'gem' => logoMakerRandomGem($random, $accent),
        'initial_cut' => logoMakerRandomInitialCut($random, $accent, $letters),
        'initial_ring' => logoMakerRandomInitialRing($random, $accent, $letters),
        'initial_axis' => logoMakerRandomInitialAxis($random, $accent, $letters),
        'initial_tiles' => logoMakerRandomInitialTiles($random, $accent, $letters),
        'initial_bracket' => logoMakerRandomInitialBracket($random, $accent, $letters),
        'initial_negative' => logoMakerRandomInitialNegative($random, $accent, $letters),
...
function logoMakerRandomInitialCut(LogoMakerRandom $random, string $accent, string $letters): string
{
...
function logoMakerRandomInitialRing(LogoMakerRandom $random, string $accent, string $letters): string
{
...
function logoMakerRandomInitialAxis(LogoMakerRandom $random, string $accent, string $letters): string
{
...
function logoMakerRandomInitialTiles(LogoMakerRandom $random, string $accent, string $letters): string
{
    $glyphA = e(mb_substr($letters, 0, 1, 'UTF-8'));
    $glyphB = e(mb_substr($letters, 1, 1, 'UTF-8')) ?: $glyphA;
...
function logoMakerRandomInitialBracket(LogoMakerRandom $random, string $accent, string $letters): string
{
...
function logoMakerRandomInitialNegative(LogoMakerRandom $random, string $accent, string $letters): string
{
...
function logoMakerRandomOrbit(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomNodes(LogoMakerRandom $random, string $accent): string
{
    $sets = [
...
function logoMakerRandomCircuit(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomPulse(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomMesh(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomHex(LogoMakerRandom $random, string $accent, string $uid): string
{
...
function logoMakerRandomBlocks(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomRingCut(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomGear(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomChevron(LogoMakerRandom $random, string $accent): string
{
    $direction = $random->pick([1, -1]);
...
function logoMakerRandomRoof(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomGrid(LogoMakerRandom $random, string $accent): string
{
    $gap = $random->pick([5, 8]);
...
function logoMakerRandomBridge(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomTower(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomCross(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomCell(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomWave(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomDna(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomHeartPulse(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomBook(LogoMakerRandom $random, string $accent): string
{
    $tilt = $random->int(-4, 4);
...
function logoMakerRandomShieldMark(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomSpark(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomPencil(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomColumns(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomBars(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomCoin(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomTrend(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomLedger(LogoMakerRandom $random, string $accent): string
{
    $rows = $random->int(3, 4);
...
function logoMakerRandomLeaf(LogoMakerRandom $random, string $accent): string
{
    $flip = $random->pick([1, -1]);
...
function logoMakerRandomBowl(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomSeedMark(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomSprout(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomWheat(LogoMakerRandom $random, string $accent): string
{
    $rotation = $random->int(-10, 10);
...
function logoMakerRandomPetal(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomRibbon(LogoMakerRandom $random, string $accent): string
{
...
function logoMakerRandomGem(LogoMakerRandom $random, string $accent): string
{
