...
状态：v1.20.1 起为授权插件（`config/blox-feature-policy.php` 五项均为 `licensed`），不随完整包分发，经插件市场受控下载。
...
五项作者端能力自 v1.20.1 起为 `licensed`。未授权时面板收起、已有内容冻结只读（旧文档保护覆盖单页/栏目/首页/模板/预览/单页历史恢复），前台照常渲染。
